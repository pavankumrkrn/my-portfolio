        ...
        <div className="">
            <div className="row justify-content-center m-5">
                {props.arr.map((i, index) => {
                    ...
                    return (
                        <>
                            <div className="col-lg-4 col-md-6 mt-3 mb-2" data-aos="flip-left" key={index}
                                data-aos-easing="ease-out-cubic"
                                data-aos-duration="1500">
                                <div className="card trans p-1">
                                    <img src={i.image} alt="" className="card-img-top pImage" />
                                    <div className="card-text">
                                        <p className="text-center text-white h5 mt-2">
                                            {i.name}
                                        </p>
                                        <div className="" data-aos="zoom-in-left"
                                            ...
                                            <div className="row p-3 justify-content-center">
                                                {(flag) ? <div className="col-6 text-center">
                                                    <button className="btn btn-dark button btn-sm bc" onClick={
                                                        () => { window.open(i.url); }
                                                    }>View App</button>
                                                </div> : null}
                                                <div className="col-6 text-center">
                                                    <button className="btn btn-outline-dark bc btn-light button btn-sm" onClick={
                                                        () => { window.open(i.gitHubLink); }
                                                    }>View Code</button>
                                                </div>
                                            </div>
                                            <hr className='aboutHr' />
                                            <p className="text-center text-white h5 mb-3">Technologies:</p>
                                            <div className="row justify-content-center pr-5 pl-5">
                                                {i.technologies.map((a, b) => {
                                                    return (
                                                        <div className="col m-auto" key={b + 'op'}>
                                                            <p className="text-white text-center"><span className={a}></span></p>
                                                        </div>)
                                                })}
                                            </div>
